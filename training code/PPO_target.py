...
datal = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/l3.csv')  # 2_r
[x_orig_l, y_orig_l, theta_orig_l, dtht_l, v_l] = process(datal)
a_l = np.zeros((len(v_l)-1, 1))

datar = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/r3.csv')  # 2_r
[x_orig_r, y_orig_r, theta_orig_r, dtht_r, v_r] = process(datar)
a_r = 2 * np.ones((len(v_r)-1, 1))

datas = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/s3.csv')  # 2_r
[x_orig_s, y_orig_s, theta_orig_s, dtht_s, v_s] = process(datas)
a_s = np.ones((len(v_s)-1, 1))

datasl = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/sl4.csv')  # 2_r
[x_orig_sl, y_orig_sl, theta_orig_sl, dtht_sl, v_sl] = process(datasl)
a_sl = np.ones((len(v_sl)-1, 1))
a_sl[-19:, 0] = 0

datasr = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/sr1.csv')  # 2_r
[x_orig_sr, y_orig_sr, theta_orig_sr, dtht_sr, v_sr] = process(datasr)
a_sr = np.ones((len(v_sr)-1, 1))
a_sr[-17:, 0] = 2

datals = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/ls3.csv')  # 2_r
[x_orig_ls, y_orig_ls, theta_orig_ls, dtht_ls, v_ls] = process(datals)
a_ls = np.zeros((len(v_ls)-1, 1))
a_ls[-7:, 0] = 1

datalr = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/lr3.csv')  # 2_r lr3
[x_orig_lr, y_orig_lr, theta_orig_lr, dtht_lr, v_lr] = process(datalr)
a_lr = np.zeros((len(v_lr)-1, 1))
a_lr[-18:, 0] = 2

datars = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/rs3.csv')  # 2_r
[x_orig_rs, y_orig_rs, theta_orig_rs, dtht_rs, v_rs] = process(datars)
a_rs = 2 * np.ones((len(v_rs)-1, 1))
a_rs[-7:, 0] = 1

datarl = pd.read_csv('D:/gr/rj/pc/Project/test/Xie-fish/data/12.25 with code/rl1.csv')  # 2_r
...
for episode in tqdm(range(episodes)):
    p = np.random.uniform(r_range_s, r_range_l)
    ...
    if eta <= -np.pi:
        eta = eta + 2 * np.pi
    state_cartesian = [- p * np.cos(eta), - p * np.sin(eta), phi]
    state = [p, phi, eta]
    episode_reward = 0
    states = []
    coordinates = []
...
plt.plot([0, episodes], [0.95, 0.95], 'r--')
plt.xlabel('Episode')
